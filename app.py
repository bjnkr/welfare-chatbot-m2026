...
if prompt := st.chat_input("질문을 입력하세요"):
    ...
    [소득인정액 정밀 계산 로직 (2026년 기준)]
                ...
                **2. 재산의 소득 환산액 (정밀 순차 차감 로직):**
                   ...
                   **[Step 3] 폭포수(Waterfall) 차감 - 공제액 이월 로직:**
                     **핵심 원칙: 공제액을 주거용 → 일반 → 금융 순서로 차감하되, 남은 공제액은 반드시 다음 단계로 이월**
                     ...
                     1. **주거용 재산 차감:**
                        ...
                        - **경우 1) 결과가 양수인 경우:**
                          * 남은 금액 × 1.04% = A
                          * 이월 공제액 = 0원 (공제액 소진)
                        - **경우 2) 결과가 음수인 경우 (예: -27,000,000):**
                          * A = 0원 (환산액 없음)
                          * **이월 공제액 = 27,000,000원** (절댓값) → **반드시 일반재산으로 이월**
                     
                     2. **일반 재산 차감:**
                        - 계산: (일반 재산 - **이월된 공제액**)
                        - **경우 1) 이월 공제액이 0원인 경우:**
                          * 일반 재산 × 4.17% = B
                        - **경우 2) 이월 공제액이 있는 경우 (예: 27,000,000원 이월):**
                          * (일반 재산 - 27,000,000) 계산
                          * 결과가 양수면: 남은 금액 × 4.17% = B, 이월 공제액 = 0원
                          * **결과가 음수면 (예: -9,000,000):** B = 0원, **이월 공제액 = 9,000,000원 → 금융재산으로 이월**
                     
                     3. **금융 재산 차감 (순서 엄수):**
                        - **1차 차감 (이월 공제액):** (금융 재산 - **이월된 공제액**)
                        - **2차 차감 (생활준비금):** 1차 차감 후 남은 금액에서 **500만 원** 추가 차감
                        - **최종 환산:** 2차 차감 후에도 남은 금액(양수) × 6.26% = C
                        
                     **예시:**
                     - 주거용: (172,000,000 - 199,000,000) = -27,000,000 → A=0, 이월=27,000,000
                     - 일반: (18,000,000 - 27,000,000) = -9,000,000 → B=0, 이월=9,000,000
                     - 금융: (30,000,000 - 9,000,000 - 5,000,000) = 16,000,000 × 6.26% = 1,001,600원
                ...
                **4. 답변 가이드:**
                   ...
                   - **재산 정보가 제공되지 않은 경우:** 재산 환산액 계산 섹션을 생략하고, "재산 정보가 제공되지 않아 소득 평가액만 계산했습니다"라고 안내할 것.
