...
if prompt := st.chat_input("질문을 입력하세요"):
    ...
    [소득인정액 정밀 계산 로직 (2026년 기준)]
                ...
                **2. 재산의 소득 환산액 (Waterfall 차감 방식 엄수):**
                   - **핵심 원칙:** 기본재산액과 부채를 합친 '총 공제 가능액'을 계산한 뒤, **주거용 → 일반 → 금융** 순서로 차감하며, **남은 공제액은 다음 재산으로 이월(Carry-over)**하여 끝까지 차감하시오.

                   - **[Step 1] 총 공제 가능액 계산:**
                     (기본재산공제액 + 부채) = **총 공제액(Pool)**

                   - **[Step 2] 순차적 차감 로직:**
                     1. **주거용 재산:** (주거용 재산 - 총 공제액)
                        👉 결과가 0원 이하라면? → 주거용 재산은 0원, **남은 공제액을 다음 단계로 넘김.**
                        👉 결과가 양수라면? → 그 금액 × 1.04% 적용. (남은 공제액 없음)
                     2. **일반 재산:** (일반 재산 - Step 1에서 넘어온 남은 공제액)
                        👉 결과가 0원 이하라면? → 일반 재산은 0원, **남은 공제액을 다음 단계로 넘김.**
                        👉 결과가 양수라면? → 그 금액 × 4.17% 적용.
                     3. **금융 재산:** ((금융 재산 - 생활준비금 500만 원) - Step 2에서 넘어온 남은 공제액)
                        👉 결과가 0원 이하라면? → 금융 재산은 0원.
                        👉 결과가 양수라면? → 그 금액 × 6.26% 적용.

                   - **[Step 3] 자동차:**
                     - 자동차는 위 공제 로직과 별개로 계산(단, 생업용/장애인 차량은 일반재산에 포함하여 계산).
